...
def buscar(log, sen):
    usuario = []
    try:
        with open('usuario(erro).txt', 'r', newline='') as arquivo:
...
while True:
    ...
    if opc == '1' or opc == 'cadastro':
        ...
        if user == True:
            ...
            with open('usuario(erro).txt', 'a+', encoding='Utf-8', newline='') as arquivo:
...
if log != 'fun' and log != '' and adeus1 != 'vá embora':
    if '2' in opc or 'login' in opc:
        with open('pedidol(erro).txt', 'a+', encoding='Utf-8', newline='') as arquivo:
            arquivo.writelines(f'{pedi}\n')
        with open('usuariol(erro).txt', 'a+', encoding='Utf-8', newline='') as arquivo:
            arquivo.writelines(f'{log}, {sen}')
    if '1' in opc or 'cadasto' in opc:
        with open('pedido(erro).txt', 'a+', encoding='Utf-8', newline='') as arquivo:
            arquivo.writelines(f'{pedi}\n')
...
if opc != '3' and opc != 'sair' and opc2 != '4' and opc2 != 'sair' and log != 'fun' and sen != '5544':
    ...
    if 'mesa' in ret or 'levar' in ret or 'entregue':
        ...
        with open('mesa(erro).txt', 'a+', encoding='Utf-8', newline='') as arquivo:
